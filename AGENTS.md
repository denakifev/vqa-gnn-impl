...
Текущая стратегия: практичные GQA и VQA-2 training-path’ы, которые можно
запускать в Kaggle-лимитах, а затем сравнивать с архитектурными изменениями.
Текущая research-ветка: additive graph-link module поверх GQA baseline. Цель
для GQA — `paper-aligned approximation`; VQA-2 — coursework extension, а не
результат статьи.

## Главные Правила

- Сначала читать структуру проекта, релевантные configs и существующие
  интерфейсы, потом менять код.
...
- Любой claim на real-data должен быть подтверждён командой и результатом
  запуска.
...
- [PROJECT_SCOPE.md](PROJECT_SCOPE.md) — явный scope репозитория.
- [ARCHITECTURE_SPLIT.md](ARCHITECTURE_SPLIT.md) — текущее разделение архитектуры.
- [GAP_ANALYSIS.md](GAP_ANALYSIS.md) — ограничения, отклонения, Kaggle-риски.
- [DATA_CONTRACTS.md](DATA_CONTRACTS.md) — контракты данных GQA и VQA-2.
- [GQA_CLOSURE_REPORT.md](GQA_CLOSURE_REPORT.md) — итог GQA engineering-фазы.
- [GRAPH_LINK_MODULE_DESIGN.md](GRAPH_LINK_MODULE_DESIGN.md) — дизайн active
  research-module и режимы экспериментов.

Режим репозитория должен оставаться явным:
...
Использовать только эти status-токены:
...
Strict GQA package был локально собран, загружен в Kaggle и затем
...
- `answer_to_idx` существует, размер `1842`;
- relation vocab существует, число relations `624`;
...
- `graph_edge_types` находится в batch contract и используется
...
- `answer_scores` находится в batch contract и используется `VQALoss` /
  `VQAAccuracy`.
...
Общее ядро должно оставаться слоем primitives, а не местом скрытого
task-switch.
...
smoke/runtime validation и соответствие Kaggle budget.
...
4. Изменения интерфейсов: batch keys, config keys, model I/O, data paths.
